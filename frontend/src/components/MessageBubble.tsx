      ...
      <div className={`max-w-[72%] ${isTenant ? "items-end" : "items-start"} flex flex-col gap-1`}>
        ...
        {!isTenant && sources.length > 0 && (
          <div className="mt-1 w-full max-w-[560px] rounded-2xl border border-ranting-sky/20 bg-ranting-navy/45 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
            <div className="ml-3 mb-2 text-[10px] font-semibold uppercase tracking-[0.22em]" style={{ color: "#1e3a5f" }}>
              Sources
            </div>
              ...
              {sources.map((source) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <ExternalLink className="mt-0.5 h-3.5 w-3.5 shrink-0" style={{ color: "#163a5e" }} />
                  </div>
                  {source.content_snippet && (
                    <p className="mt-1 line-clamp-2 text-[11px] leading-4" style={{ color: "#1e3a5f" }}>
                      {source.content_snippet}
                    </p>
                  )}
                  <div className="mt-2 truncate text-[10px]" style={{ color: "#1a4060" }}>
                    {source.url}
                  </div>
                </a>
              ))}
