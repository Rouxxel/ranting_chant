    ...
    const fetchStats = async () => {
      try {
        ...
        const managedTenants = tenants.filter(
          (t) => t.property_id && managedPropertyIds.includes(t.property_id),
        );

        // Filter requests by managed properties
        const managedRequests = requests.filter(
          (r) => r.property_id && managedPropertyIds.includes(r.property_id),
        );

        // Calculate stats
        const activeRequests = managedRequests.filter(
        ...
        const resolvedThisWeek = managedRequests.filter(
          (r) => r.status === "resolved" && r.resolved_at && new Date(r.resolved_at) >= oneWeekAgo,
        ).length;

        // Calculate average response time (simplified)
        const resolvedRequests = managedRequests.filter(
        ...
        if (resolvedRequests.length > 0) {
          ...
          avgResponseTime =
            avgHours < 1 ? `${Math.round(avgHours * 60)} min` : `${avgHours.toFixed(1)} hours`;
        }
